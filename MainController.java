...
public class MainController 
{
	...
	int PAGE_SIZE = 5;
	@GetMapping("/emplist")
	public String openEmpListPage(Model model, @RequestParam(defaultValue = "1") int page, @RequestParam(name="redirectAttri_success", required=false)String success,
			@RequestParam(name="redirectAttri_error",required=false)String error)
	{
	...
	public String deleteEmployee(@RequestParam("empEmail") String email, RedirectAttributes redirectAttributes)
	{
		...
		if(status)
		{
			redirectAttributes.addAttribute("redirectAttri_success", "Employee deleted successfully");
		}
		else
		{
			redirectAttributes.addAttribute("redirectAttri_error", "Employee not deleted due to some error");
		}
